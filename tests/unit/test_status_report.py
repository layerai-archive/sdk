...
def test_sql_status_report_to_json() -> None:
    message = "Test msg"
    cause = "Test cause"
    expected_dict = {"message": message, "cause": cause, "type": "SQL"}
    actual_json = ExecutionStatusReportFactory.to_json(
        SQLExecutionStatusReport(message, cause)
    )
    actual_dict = json.loads(actual_json)
    assert expected_dict == actual_dict


def test_sql_status_report_from_json() -> None:
    message = "Test msg"
    cause = "Test cause"
    json = f'{{"message": "{message}",' f'"type": "SQL",' f'"cause": "{cause}"}}'
    expected = SQLExecutionStatusReport(message, cause)
    actual = ExecutionStatusReportFactory.from_json(json)
    assert expected == actual
